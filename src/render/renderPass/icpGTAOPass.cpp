...
{
	m_pipelineInfo.m_pipeline = ComputePipelineBuilder(m_rhi)
...
void icpGTAOPass::Dispatch(uint32_t frameBufferIndex, uint32_t currentFrame, VkResult acquireImageResult)
{
	...
	uint32_t groupsX = ((float)m_rhi->GetSwapChainExtent().width / 2.f + 16 - 1) / 16;
	uint32_t groupsY = ((float)m_rhi->GetSwapChainExtent().width / 2.f + 16 - 1) / 16;
	uint32_t groupsZ = 1;
...
void icpGTAOPass::CreateAORenderTarget()
{
	icpVulkanUtility::CreateGPUImage(
		(float)m_rhi->GetSwapChainExtent().width / 2.f,
		(float)m_rhi->GetSwapChainExtent().height / 2.f,
		1,
		1,
		VK_FORMAT_R8_UNORM,
